...
public abstract class Mod {
	...
	public static void initMods() {
		...
		Class<?>[] MOD_LIST = new Class<?>[] { AuthMod.class, Base10Mod.class, DataMod.class,
											   ...
											   ListMod.class, NowMod.class, OptionMod.class, 
		...
		for (Class<?> c : MOD_LIST) {
			...
			if(Mod.class.isAssignableFrom(c)) {
				...
				try {
					...
					// holy cow there are a lot of exceptions here lol
					System.err.println("\tFailed to load mod: " + c.getSimpleName());
					continue;
				}
				
				//Checks if the mod is using aliases already in use by other mods.
				//This is quite dumb, as any intersection between two sets of aliases
				//is not allowed, however I don't care enough to fix it.
				registerMod(mod);
			}
		}
	}
	...
	//May be called by external code to register a custom mod
	...
	protected Mod(String help, String[] aliases, String... author) {
		if(author.length < 0) { 
			this.author = "N/A"; 
		} else {
			this.author = String.join(", ", author).replace("`", "").replace("*", "").replace("_", "").trim();
		}
		this.help = help;
